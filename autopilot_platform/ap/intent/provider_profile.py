...
- DeepSeek 识图：仅 ``deepseek-v4-flash-vision-exp`` 等 vision 型号接受 image_url
  （https://api-docs.deepseek.com/zh-cn/guides/vision ）；v4-flash / v4-pro 仍纯文本
...
# DeepSeek 官方视觉实验模型（Chat 默认仍走纯文本 v4-flash / v4-pro）
...
def deepseek_model_accepts_images(model: str) -> bool:
    """DeepSeek 仅 vision 型号接受 image_url；v4-flash / v4-pro / chat 仍是纯文本。"""
...
def model_accepts_images(provider: str, model: str, *, base_url: str = "") -> bool:
    ...
    if p == "deepseek" or m.startswith("deepseek"):
        return deepseek_model_accepts_images(m)
    if p == "ollama":
