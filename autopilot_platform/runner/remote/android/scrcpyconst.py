...
# configured centrally in ``runner.remote.config`` (SCRCPY_SERVER_VERSION /
...
# Event
EVENT_INIT = "init"
EVENT_FRAME = "frame"          # decoded BGR ndarray (snapshot path)
EVENT_AUDIO = "audio"
EVENT_DISCONNECT = "disconnect"
EVENT_H264 = "h264"            # raw H.264 Annex-B bytes (WebRTC passthrough)
...
# (12–16 are UHID / hard-keyboard-settings / start_app opcodes that the
# bundled scrcpy-server.jar supports but this Python client doesn't.)
# TYPE_RESET_VIDEO triggers ``surfaceCapture.getCaptureControl().reset(
# RESET_REASON_CLIENT_RESET)`` on the server side — MediaCodec emits a
# fresh SPS / PPS / IDR while the TCP stream stays open and the scrcpy
# process stays up. This is the non-destructive force-keyframe path:
# no PeerConnection teardown, no ~500 ms reconnect blip. See
# services/quality_controller.py for how the adaptive controller uses it.
TYPE_RESET_VIDEO = 17
