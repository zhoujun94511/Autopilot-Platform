...
> This covers the **Mac/Xcode-side WDA build, signing & install** (a one-time prerequisite). Once WDA is installed, for starting the tunnel, launching WDA and wiring up Appium at runtime on **Windows without a Mac** via go-ios + pymobiledevice3, see [iOS setup](../setup/ios.md).
